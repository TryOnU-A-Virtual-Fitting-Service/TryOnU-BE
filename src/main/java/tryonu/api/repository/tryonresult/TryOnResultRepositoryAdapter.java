...
public class TryOnResultRepositoryAdapter implements TryOnResultRepository {
    ...
    public List<TryOnResult> findAllByUserIdWithDetailsAndIsDeletedFalseOrThrow(@NonNull Long userId) {
        ...
        log.debug("[TryOnResultRepositoryAdapter] 사용자별 피팅 결과 상세 조회 - userId: {}, count: {}", userId,
                tryOnResults.size());
        return tryOnResults;
    }
    ...
    public List<TryOnResultDto> findTryOnResultsByUserIdOrderByIdDesc(@NonNull Long userId) {
        ...
        log.debug("[TryOnResultRepositoryAdapter] 사용자별 피팅 결과 DTO 조회 - userId: {}, count: {}", userId,
                tryOnResults.size());
        return tryOnResults;
    }
}