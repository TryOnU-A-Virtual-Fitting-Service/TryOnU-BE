...
public interface JpaTryOnResultRepository extends JpaRepository<TryOnResult, Long> {
       ...
       /**
        * tryOnJobId로 피팅 결과 조회
        */
       Optional<TryOnResult> findByTryOnJobId(String tryOnJobId);

       /**
        * tryOnJobId로 삭제되지 않은 피팅 결과 조회
        */
       Optional<TryOnResult> findByTryOnJobIdAndIsDeletedFalse(String tryOnJobId);
        ...
        */
       @Query("SELECT new tryonu.api.dto.responses.TryOnResultDto(tr.tryOnJobId, tr.imageUrl, tr.defaultModelId, " +
